...
Dialog.addCheckbox("Remove outlayers", false);
...
Dialog.addCheckbox("Line ROI", false);
...
Dialog.addCheckbox("Invert image",false);
Dialog.show();
...
print("Remove outlayer: "+Oul);
radOuL=Dialog.getNumber();
print("Remove outlayer radius: "+radOuL);
ThOuL=Dialog.getNumber();
print("Remove outlayer threshold: "+ThOuL);
...
function analyze(){
	...
	getDimensions(widthQ, heightQ, channels, slicesQ, framesQ);
...
function Thresholder(){
	...
	if (roiuse==true && eachRoi==true){
		...
		for(ro=0; ro<numberRo; ro++){
			...
			if(LRoi==true){
				if(selectionType()==5 || selectionType()==6 || selectionType()==7) run("Line to Area");
